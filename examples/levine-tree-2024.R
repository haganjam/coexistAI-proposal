...
# Levine et al. (TREE, 2024)

# similar to Levine et al. architecture as far as I can tell
# without deep learning approaches though
# work with the species pool and they don't use the individual species as
# predictors... 

# combine these two frameworks
...
for (j in seq_len(n_sp)) {
  ...
  # trait value correlated with lambda
  trait_mu <- (lambda * 0.3)
  
  # add noise
  trait_val <- rnorm(n = length(trait_mu), trait_mu, sd = 1)
...
# get species abundance variables
df_out_sp <-
  df_out |>
...
df_out_trait <-
  df_out |>
  dplyr::select(plot_id, species, trait_value) |>
  tidyr::pivot_wider(names_from = "species",
                     values_from = "trait_value")
names(df_out_trait)[-1] <- paste0(names(df_out_trait)[-1], "_trait")
...
# mean trait in the community
df_out$CWM <-
  apply(df_out[, names(df_out)[grepl(pattern = "_trait", names(df_out))]], 1, mean)

# calculate cv of the traits
df_out$FD <-
  apply(df_out[, names(df_out)[grepl(pattern = "_trait", names(df_out))]], 1, function(x) {
    sd(x)/mean(x)
  })
...
X <- 
  ...
  dplyr::select(env_var, 
                dplyr::all_of(sp_names),
                CWM)  # only predictors, not response
